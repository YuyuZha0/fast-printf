...
import static io.fastprintf.util.internal.MathUtils.*;
import static io.fastprintf.util.internal.MultiplyHigh.multiplyHigh;
...
public final class DoubleToDecimal {
  ...
  /**
   * Splits the decimal <i>d</i> described in {@link Double#toString(double)} in integers <i>f</i>
   * and <i>e</i> such that <i>d</i> = <i>f</i> 10<sup><i>e</i></sup>.
   *
   * <p>Further, determines integer <i>n</i> such that <i>n</i> = 0 when <i>f</i> = 0, and
   * 10<sup><i>n</i>-1</sup> &le; <i>f</i> &lt; 10<sup><i>n</i></sup> otherwise.
   *
   * <p>The argument {@code v} is assumed to be a positive finite value or positive zero. Further,
   * {@code fd} must not be {@code null}.
   *
   ...
   *
   * <p>The outcome is the same as if {@code v} were first {@link #toString(double) rendered} and
   * the resulting string were then {@link Appendable#append(CharSequence) appended} to {@code app}.
   *
  ...
  private int toDecimal(double v, FormattedFPDecimal fd) {
    ...
    if (bq < BQ_MASK) {
      ...
      if (t != 0) {
        /* subnormal value */
        return t < C_TINY ? toDecimal(Q_MIN, 10 * t, -1, fd) : toDecimal(Q_MIN, t, 0, fd);
      }

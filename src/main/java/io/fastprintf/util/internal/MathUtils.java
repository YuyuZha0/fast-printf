...
final class MathUtils {
  ...
  /**
   * Returns the unique integer <i>k</i> such that 10<sup><i>k</i></sup> &le; 2<sup>{@code e}</sup>
   * &lt; 10<sup><i>k</i>+1</sup>.
   *
   * <p>The result is correct when |{@code e}| &le; 6_432_162. Otherwise the result is undefined.
   *
   * @param e The exponent of 2, which should meet |{@code e}| &le; 6_432_162 for safe results.
   ...
   *
   * <p>The result is correct when -3_606_689 &le; {@code e} &le; 3_150_619. Otherwise the result is
   * undefined.
   *
   * @param e The exponent of 2, which should meet -3_606_689 &le; {@code e} &le; 3_150_619 for safe
   *     results.
  ...
  /**
   * Returns the unique integer <i>k</i> such that 2<sup><i>k</i></sup> &le; 10<sup>{@code e}</sup>
   * &lt; 2<sup><i>k</i>+1</sup>.
   *
   ...
   * 2<sup>126</sup>. Further, let <i>g</i> = &lfloor;<i>&beta;</i>&rfloor; + 1. Split <i>g</i> into
   * the higher 63 bits <i>g</i><sub>1</sub> and the lower 63 bits <i>g</i><sub>0</sub>. Thus,
   * <i>g</i><sub>1</sub> = &lfloor;<i>g</i> 2<sup>-63</sup>&rfloor; and <i>g</i><sub>0</sub> =
   * <i>g</i> - <i>g</i><sub>1</sub> 2<sup>63</sup>.
   *
   * <p>This method returns <i>g</i><sub>1</sub> while {@link #g0(int)} returns
   * <i>g</i><sub>0</sub>.
   *
   * <p>If needed, the exponent <i>r</i> can be computed as <i>r</i> = {@code flog2pow10(-k)} - 125
   * (see {@link #flog2pow10(int)}).
   *
   * @param k The exponent of 10, which must meet {@link #K_MIN} &le; {@code e} &le; {@link #K_MAX}.

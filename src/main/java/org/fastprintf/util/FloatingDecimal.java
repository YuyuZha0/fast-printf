...
/**
 * A class for converting between ASCII and decimal representations of a single or double precision
 * floating point number. Most conversions are provided via static convenience methods, although a
 * <code>BinaryToASCIIConverter</code> instance may be obtained and reused.
 */
public final class FloatingDecimal {
  ...
  static final FloatingDecimal.ASCIIToBinaryConverter A2BC_POSITIVE_INFINITY =
      new FloatingDecimal.PreparedASCIIToBinaryBuffer(
          Double.POSITIVE_INFINITY, Float.POSITIVE_INFINITY);
  static final FloatingDecimal.ASCIIToBinaryConverter A2BC_NEGATIVE_INFINITY =
      new FloatingDecimal.PreparedASCIIToBinaryBuffer(
          Double.NEGATIVE_INFINITY, Float.NEGATIVE_INFINITY);
  static final FloatingDecimal.ASCIIToBinaryConverter A2BC_NOT_A_NUMBER =
      new FloatingDecimal.PreparedASCIIToBinaryBuffer(Double.NaN, Float.NaN);
  static final FloatingDecimal.ASCIIToBinaryConverter A2BC_POSITIVE_ZERO =
      new FloatingDecimal.PreparedASCIIToBinaryBuffer(0.0d, 0.0f);
  static final FloatingDecimal.ASCIIToBinaryConverter A2BC_NEGATIVE_ZERO =
      new FloatingDecimal.PreparedASCIIToBinaryBuffer(-0.0d, -0.0f);
  ...
  private static final FloatingDecimal.BinaryToASCIIConverter B2AC_POSITIVE_INFINITY =
      new FloatingDecimal.ExceptionalBinaryToASCIIBuffer(INFINITY_REP, false);
  private static final FloatingDecimal.BinaryToASCIIConverter B2AC_NEGATIVE_INFINITY =
      new FloatingDecimal.ExceptionalBinaryToASCIIBuffer("-" + INFINITY_REP, true);
  private static final FloatingDecimal.BinaryToASCIIConverter B2AC_NOT_A_NUMBER =
      new FloatingDecimal.ExceptionalBinaryToASCIIBuffer(NAN_REP, false);
  private static final FloatingDecimal.BinaryToASCIIConverter B2AC_POSITIVE_ZERO =
      new FloatingDecimal.BinaryToASCIIBuffer(false, new char[] {'0'});
  private static final FloatingDecimal.BinaryToASCIIConverter B2AC_NEGATIVE_ZERO =
      new FloatingDecimal.BinaryToASCIIBuffer(true, new char[] {'0'});
  private static final ThreadLocal<FloatingDecimal.BinaryToASCIIBuffer>
      threadLocalBinaryToASCIIBuffer =
          new ThreadLocal<FloatingDecimal.BinaryToASCIIBuffer>() {
            @Override
   ...
   * @throws NumberFormatException If the <code>String</code> does not represent a properly
   *     formatted double precision value.
   */
   ...
   * @throws NumberFormatException If the <code>String</code> does not represent a properly
   *     formatted single precision value.
   */
  ...
  /**
   * Returns a <code>BinaryToASCIIConverter</code> for a <code>double</code>. The returned object is
   * a <code>ThreadLocal</code> variable of this class.
   *
   * @param d The double precision value to convert.
   * @return The converter.
   */
  public static FloatingDecimal.BinaryToASCIIConverter getBinaryToASCIIConverter(double d) {
   ...
   * @return The converter.
   */
  static FloatingDecimal.BinaryToASCIIConverter getBinaryToASCIIConverter(
    ...
    FloatingDecimal.BinaryToASCIIBuffer buf = getBinaryToASCIIBuffer();
  ...
  private static FloatingDecimal.BinaryToASCIIConverter getBinaryToASCIIConverter(float f) {
  ...
  static FloatingDecimal.ASCIIToBinaryConverter readJavaFormatString(String in)
  ...
  /**
   * Converts string s to a suitable floating decimal; uses the double constructor and sets the
   * roundDir variable appropriately in case the value is later converted to a float.
   *
   * @param s The <code>String</code> to parse.
   */
  static FloatingDecimal.ASCIIToBinaryConverter parseHexString(String s) {
    // Verify string is a member of the hexadecimal floating-point
    // string language.
    Matcher m = FloatingDecimal.HexFloatPattern.VALUE.matcher(s);
    ...
    if (!validInput) {
      ...
      if (exponent >= Float.MIN_EXPONENT) {
        ...
        if (exponent < FloatConsts.MIN_SUB_EXPONENT - 1) {
          ...
          int threshShift =
              (int)
                  ((DoubleConsts.SIGNIFICAND_WIDTH - 2 + FloatConsts.MIN_SUB_EXPONENT) - exponent);
      ...
      if (exponent > Double.MAX_EXPONENT) { // Infinite result
        ...
        if (exponent <= Double.MAX_EXPONENT
          ...
          significand =
              (((exponent + (long) DoubleConsts.EXP_BIAS) << (DoubleConsts.SIGNIFICAND_WIDTH - 1))
  ...
  /**
   * A converter which can process an ASCII <code>String</code> representation of a single or double
   * precision floating point value into a <code>float</code> or a <code>double</code>.
   */
  ...
  static class BinaryToASCIIBuffer implements FloatingDecimal.BinaryToASCIIConverter {
    ...
    /**
     * Default constructor; used for non-zero values, <code>BinaryToASCIIBuffer</code> may be
     ...
     *
     * <p>First, scale the mantissa bits such that 1 <= d2 < 2. We are then going to estimate
     * log10(d2) ~=~ (d2-1.5)/1.5 + log(1.5) and so we can estimate log10(d) ~=~ log10(d2) + binExp
     * * log10(2) take the floor and call it decExp.
     */
    static int estimateDecExp(long fractBits, int binExp) {
      ...
      if (exponent >= 0 && exponent < 52) { // hot path
        ...
        int r =
            (int) (((dBits & DoubleConsts.SIGNIF_BIT_MASK) | FRACT_HOB) >> (EXP_SHIFT - exponent));
    ...
    /**
     * This is the easy subcase -- all the significant bits, after scaling, are held in lvalue.
     * negSign and decExponent tell us what processing and scaling has already been done.
     * Exceptional cases have already been stripped out. In particular: lvalue is a finite number
     * (not Inf, nor NaN) lvalue > 0L (not zero, nor negative).
     *
     * <p>The only reason that we develop the digits here, rather than calling on Long.toString() is
     * that we can do it a little faster, and besides want to treat trailing 0s specially. If
    ...
    private void developLongDigits(int decExponent, long lvalue, int insignificantDigits) {
      if (insignificantDigits != 0) {
        ...
        long pow10 =
            FDBigInteger.LONG_5_POW[insignificantDigits]
  ...
  static class ASCIIToBinaryBuffer implements FloatingDecimal.ASCIIToBinaryConverter {
    ...
    /**
     * Takes a FloatingDecimal, which we presumably just scanned in, and finds out what its value
     * is, as a double.
     *
     * <p>AS A SIDE EFFECT, SET roundDir TO INDICATE PREFERRED ROUNDING DIRECTION in case the result
     * is really destined for a single-precision float.
     */
    @Override
    public double doubleValue() {
      ...
      final int B5 =
          Math.max(0, -exp); // powers of 5 in bigB, value is not modified inside correctionLoop
      final int D5 =
          Math.max(0, exp); // powers of 5 in bigD, value is not modified inside correctionLoop
    ...
    /**
     * Takes a FloatingDecimal, which we presumably just scanned in, and finds out what its value
     * is, as a float. This is distinct from doubleValue() to avoid the extremely unlikely case of a
     * double rounding error, wherein the conversion to double has one rounding error, and the
     * conversion of that double to a float has another rounding error, IN THE WRONG DIRECTION, (
  ...
  private static class HexFloatPattern {
    /**
     * Grammar is compatible with hexadecimal floating-point constants described in section 6.4.4.2

...
*   **Glibc vs. Java `Formatter` Conventions**: Follows `glibc` `printf`. For example, `%S` converts the entire string to uppercase, unlike Java's behavior.
...
*   **Type-Safe `%p`**: The `%p` specifier works only for object references and will correctly throw an exception if given a primitive type, preventing bugs related to auto-boxing.
